...
if (!$dge) {			
	...
	if($user_id){			
		...
		$message = "User display name: {$user->display_name}<br />User email: {$user->user_email}<br />User ID: {$user->ID}<br />Status: " . $status . 
		           "<br />Action: Purchase post:" . $subject . "<br /><br />" . $message . "<br /><br /><pre>" . print_r($_POST, true) . '</pre>';
	}else{
...
if($subscr_id)
	$message .= "<br/><br/>" .__('Client Id: ','mgm' ) . $subscr_id;
if(isset($member->payment_info->txn_id))	
	$message .= "<br/>" .__('Order Id:' ,'mgm' ) . $member->payment_info->txn_id;
if(isset($member->payment_info->transaction_id))			
