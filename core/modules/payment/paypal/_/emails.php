...
if(!$dge){
	// message
	$message = 'Could not read membership type in the following POST data. Please debug or contact magic members to fix the problem 
	            making sure to pass on the following data. <br /><br /><pre>' . "\n\n" . print_r($_POST, true) . '</pre>';
	// mail
...
// notify once
if($_POST['txn_type'] == 'subscr_payment' || ( $_POST['txn_type'] == 'subscr_signup' && ($_POST['payment_status'] == 'Processed' || $_POST['payment_status'] == 'Completed'))){ 
	// notify user
	if(!$dpne) {
		// check
		if($acknowledge_user) {
...
if(!$dge) {					
	...
	if($subscr_id)
		$message .= "<br/><br/>" .__('Profile Id: ','mgm' ) . $subscr_id;
