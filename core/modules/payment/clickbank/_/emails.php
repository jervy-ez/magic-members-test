...
// notify user
if(!$dpne) {
	if($user_id && $this->send_payment_email($passthrough['custom'])) {
		//issue #862
		...
		$this->update_paymentemail_sent($passthrough['custom']);	
	}
}
...
if ($tran_success) {
	...
	if(isset($guest_token)){
		...
		if(isset($coupon_id) && isset($coupon_code)) {
			...
			$this->_set_purchased(NULL, $post_id, $guest_token, $passthrough['custom'],$coupon_code);
		}else {
			$this->_set_purchased(NULL, $post_id, $guest_token, $passthrough['custom']);				
		}
	}else{
		$this->_set_purchased($user_id, $post_id, NULL, $passthrough['custom']);
	}
	// status
...
if(!$dge){
	...
	mgm_mail($system_obj->setting['admin_email'], 'Error in Clickbank membership verification', $message);
}
...
$user = get_userdata($user_id);
			...
			$message .= "<br/>" .__('Please unsubscribe the user from Gateway Merchant.', 'mgm');
			if($subscr_id)
				$message .= "<br/><br/>" .__('Transaction Id: ','mgm' ) . $subscr_id;			
