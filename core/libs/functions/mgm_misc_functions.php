...
function mgm_log($data, $filename=NULL, $suffix=true){
	...
	if($suffix){
		// define
		if(!defined('MGM_LOG_REQUEST_ID')) define('MGM_LOG_REQUEST_ID', date('mdYHis'));
		 // file name 
		...
		if ($allow_on_purchasable && $purchasable) {// if purchasable
			...
			if ($return == true && $user->ID > 0) {				
				...
				if(	in_array( strtolower($member->membership_type), $user_membership_types ) && 
					...
					in_array(strtolower($member->membership_type),$allowed_membership_types)) {
...
function mgm_get_currencies(){
	...
	$currencies = array('AED' => sprintf(__('%s - Arab Emirates Dirham','mgm'), 'AED'),'ARS' => sprintf(__('%s -  Argentina Peso ','mgm'), 'ARS'),
		...
		'PLN' => sprintf(__('%s - Polish Zloty','mgm'), 'PLN'),'RUB' => sprintf(__('%s - Russian ruble','mgm'), 'RUB'),
		'SEK' => sprintf(__('%s - Swedish Krona','mgm'), 'SEK'),'SGD' => sprintf(__('%s - Singapore Dollar','mgm'), 'SGD'),	
		'THB' => sprintf(__('%s - Thai Baht','mgm'), 'THB'),'TRY' => sprintf(__('%s - Turkish Lira','mgm'), 'TRY'),	
		'TWD' => sprintf(__('%s - New Taiwanese Dollar','mgm'), 'TWD'),'USD' => sprintf(__('%s - U.S. Dollar','mgm'), 'USD'),
		'ZAR' => sprintf(__('%s - South African Rand','mgm'), 'ZAR'));			
	// return	
...
function mgm_get_currency_symbols($code=NULL){
	// symbols
	$symbols = array('AED' => 'د.إ','ARS' => '&#36;','AUD' => '&#36;','BRL' => 'R$','CAD' => '&#36;','CHF' => '&#8355;','CZK' => 'Kč','DKK' => 'kr','EUR' => '&#128;',
...
function mgm_get_currency_iso4217($code=NULL){
	// currencies
	$currencies = array('AED' => 784,'ARS' => 032,'AUD' => 036,'BRL' => 986,'CAD' => 124,'CHF' => 756,'CZK' => 203,'DKK' => 208, 'EUR' => 978, 
